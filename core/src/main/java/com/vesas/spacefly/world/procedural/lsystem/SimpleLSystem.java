...
public class SimpleLSystem
{
	...
	private static SimpleLSystem INSTANCE = new SimpleLSystem();
	
	public static SimpleLSystem getInstance()
	{
		INSTANCE.init();
		return INSTANCE;
	}
	...
	private void renderElement( Pixmap pm, StackElement elem )
	{
		...
		StackElement newElem2 = new StackElement();
		newElem2.prevLevelTip.x = elem.start.x + elem.dir.x;
		newElem2.prevLevelTip.y = elem.start.y + elem.dir.y;
		
		newElem2.start.x = elem.start.x + elem.dir.x * 0.89f;
		newElem2.start.y = elem.start.y + elem.dir.y * 0.89f;
		
		tmp.x = elem.dir.x * scaling;
		tmp.y = elem.dir.y * scaling;
		tmp.rotateDeg( 46 );
		newElem2.dir.x = tmp.x;
		newElem2.dir.y = tmp.y;
		newElem2.level = elem.level + 1;
